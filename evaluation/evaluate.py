...
try:
    ...
    import evaluation_qk.evaluation as eva
...
class AudioTaggingEvaluate(object):
    ...
    def get_stats_from_prob_mat_csv(self, pd_prob_mat_csv, thres_ary):
        ...
        #todo this whole function should get checked
        #print(self.weak_gt_csv, "self.weak_gt_csv")
        (gt_na_list, gt_mat) = at_read_weak_gt_csv(self.weak_gt_csv, self.lbs)
        #print("gt_na_list", gt_na_list, "gt_mat", gt_mat)#gta_na_list isnt empty if we add "Y" in at_read_weak_gt_csv() but that may be a quick fix
        #print(pd_prob_mat_csv)#preds/crnn_sed/at_prob_mat.csv.gz
        (pd_na_list, pd_prob_mat) = at_read_prob_mat_csv(pd_prob_mat_csv)#pd_prob mat isn empty dec132018
        #print("pd_prob_mat", pd_prob_mat, "pd_na_list", pd_na_list, "gt_na_list", gt_na_list)
        #print("pd_na_list, pd_prob_mat", pd_na_list, pd_prob_mat)#not empty and a big list of names!
        pd_prob_mat = self._reorder_mat(pd_prob_mat, pd_na_list, gt_na_list)
        #print("pd_prob_mat", pd_prob_mat)
        #print(pd_prob_mat, "pd_prob_mat")#empty | still empty todo
        del pd_na_list
        #print("pd_prob_mat", pd_prob_mat)#todo
    ...
    def get_stats_from_prob_mat(self, pd_prob_mat, gt_mat, thres_ary):
        ...
        """        
        n_lbs = len(self.lbs)
        #print("pd_prob_mat", pd_prob_mat, "gt_mat", gt_mat)#todo
        stat = {}
        ...
        # overall stat
        #print("pd_prob_mat", pd_prob_mat, "pd_prob_mat.flatten()", pd_prob_mat.flatten(), "gt_mat", gt_mat, "gt_mat.flatten()", gt_mat.flatten(), "finn")#todo likely there is still an issue with .flatten and sizes. Should look into this
    ...
    def _reorder_mat(self, pd_mat, pd_na_list, gt_na_list):
        indexes=[]
        finns_badcodehabits=True
        if finns_badcodehabits==True:#lol #finn changed this. hopefully nothing bad will happend as a result down the line if you use it
            for e in gt_na_list:
                if e in pd_na_list:
                    indexes.append(pd_na_list.index(e))
                else:
                    #print(e, "finn")
                    for x in pd_na_list:
                        if e in x:
                            indexes.append(pd_na_list.index(x))#hmm using index(x) is risky.
                            #print(x, "finn look!")
        else:  
            #print(pd_na_list, "pd_na_list Finn")#big list of names
            #print(gt_na_list, "gt_na_list Finn")
            
            indexes = [pd_na_list.index('Y'+e) for e in gt_na_list]#had to add Y because that is how it is done in testing set... finn #todo this is probably gonna have to be fixed if we remove finnsbadcodehabits
            #print(indexes, "indexes finn")
    ...
    def _get_digit_mat_from_thres_ary(self, pd_prob_mat, thres_ary):
        ...
        for n in range(len(pd_prob_mat)):
            for k in range(len(self.lbs)):
                #removed the below print statements since they are from trying to get 3classes to work dec4
                #print("pd_prob_mat.shape(), thres_ary.shape()", pd_prob_mat.shape)#, thres_ary.shape())
                #print("pd_prob_mat.shape(), thres_ary.shape()",len(thres_ary))#, thres_ary.shape())
                #print("n , k", n, k)#so prob mat needs to be a 3 col matrix
...
class SoundEventDetectionEvaluate(object):
    ...
    def _reorder_mat_list(self, pd_mat_list, pd_na_list, gt_na_list):
        indexes=[]
        finns_badcodehabits=True
        if finns_badcodehabits==True:#lol #finn changed this. hopefully nothing bad will happend as a result down the line if you use it #todo
            #print("gt_na_list finn", gt_na_list)
            for e in gt_na_list:
                if e in pd_na_list:
                    indexes.append(pd_na_list.index(e))
                else:
            ...
            #print(pd_na_list, "pd_na_list Finn")#big list of names
            #print(gt_na_list, "gt_na_list Finn")
            indexes = [pd_na_list.index('Y'+e) for e in gt_na_list]#had to add Y because that is how it is done in testing set... finn
            #print(indexes, "indexes finn")
            ##FINN be aware that this is just a copy of the other isntance of finns_badcodehabits in this file.(:
